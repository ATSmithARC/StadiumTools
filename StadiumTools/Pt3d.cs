...
namespace StadiumTools
{
    ...
    public struct Pt3d : ICloneable
    {
        ...
        /// <summary>
        /// calculates point between two points at a parameter for an array of points
        /// </summary>
        /// <param name="a"></param>
        /// <param name="b"></param>
        /// <param name="t"></param>
        ...
        public static Pt3d[] Tween2(Pt3d[] a, Pt3d[] b, double t)
        {
            ...
            {
                throw new ArgumentException($"Error: Tween2 must have inputs of equal length [{a.Length}, {b.Length}]");
            }
            Pt3d[] result = new Pt3d[a.Length];
            for (int i = 0; i < result.Length; i++)
            {
                Vec3d AB = new Vec3d(b[i] - a[i]);
                Vec3d AC = (t * AB.M) * Vec3d.Normalize(AB);
                Pt3d c = a[i] + AC;
                result[i] = c;
            }
        ...
        public static Pt3d[][] Tween2(Pt3d[] a, Pt3d[] b, double[] t)
        {
            ...
            Pt3d[][] result = new Pt3d[t.Length][];
            for (int i = 0; i < t.Length; i++)
            {
                result[i] = new Pt3d[a.Length];
                for (int j = 0; j < a.Length; j++)
                {
                    Vec3d AB = new Vec3d(b[j] - a[j]);
                    Vec3d AC = (t[i] * AB.M) * Vec3d.Normalize(AB);
                    Pt3d c = a[j] + AC;
                    result[i][j] = c;
                }
            }
